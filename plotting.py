...
def spatiotemporal(nt,car_ids,L):
    ...
    for i in range(len(car_ids)):
        ...
        if "t_f" in car_ids[i]:
            rect = mpatches.Rectangle([car_ids[i]["loc"]-car_ids[i]["len"],car_ids[i]["t_i"]], 2*car_ids[i]["len"], car_ids[i]["t_f"]-car_ids[i]["t_i"], ec="none")
        else:
            rect = mpatches.Rectangle([car_ids[i]["loc"]-car_ids[i]["len"],car_ids[i]["t_i"]], 2*car_ids[i]["len"], nt-car_ids[i]["t_i"], ec="none")
        patches.append(rect)
