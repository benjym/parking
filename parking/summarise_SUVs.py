...
for i, parking_mode in enumerate(["random"]):
    for j, s in enumerate(params["sigma_car_length"]):
        ...
        d = np.load(
            f"output/parking_mode_{parking_mode}/L_{params['L'][0]}/mean_car_length_{params['mean_car_length'][0]}/sigma_car_length_{s}/linear_density.npy"
        )
        ax[0].semilogx(
...
for i, parking_mode in enumerate(["one_side"]):
    for j, s in enumerate(params["sigma_car_length"]):
        c = cm.plasma(j / len(params["sigma_car_length"]))
        d = np.load(
            f"output/parking_mode_{parking_mode}/L_{params['L'][0]}/mean_car_length_{params['mean_car_length'][0]}/sigma_car_length_{s}/cars.npy"
        )
        ax[3].hist(
            d,
            label=f"$\sigma={s}$ m",
